...
fun GoogleAnalytics.Companion.Http(measurementId: MeasurementId, apiSecret: ApiSecret, rawHttp: HttpHandler = JavaHttpClient().debug()) =
    object : GoogleAnalytics {
        private val http = SetBaseUriFrom(GOOGLE_ANALYTICS_URL).then(rawHttp)

        override fun <R> invoke(action: GoogleAnalyticsAction<R>): Result<R, RemoteFailure> {
