...
val AWS_CONTAINER_CREDENTIALS_FULL_URI = EnvironmentKey.uri()
    .map({
        when {
            it.toString().startsWith("http") -> it
            else -> STANDARD_CC_BASE_URL.extend(it)
        }
    }, { Uri.of(it.toString().removePrefix(STANDARD_CC_BASE_URL.toString())) })
    .defaulted("AWS_CONTAINER_CREDENTIALS_FULL_URI", AWS_CONTAINER_CREDENTIALS_RELATIVE_URI)

private val STANDARD_CC_BASE_URL = Uri.of("http://169.254.170.2")

val AWS_CONTAINER_AUTHORIZATION_TOKEN = EnvironmentKey.value(ContainerCredentialsAuthToken).required("AWS_CONTAINER_AUTHORIZATION_TOKEN")
