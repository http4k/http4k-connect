...
fun bucketPutBucket(buckets: Storage<Unit>) = "/" bind Method.PUT to { putBucket(it.subdomain(buckets), buckets) }

fun globalPutBucket(buckets: Storage<Unit>) =
    "/{bucketName}" bind Method.PUT to { putBucket(it.path("bucketName")!!, buckets) }
