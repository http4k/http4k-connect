...
abstract class S3BucketContract {
    ...
    fun `bucket key lifecycle`() {
        ...
        assertThat(s3Bucket.get(key).successValue().let { String(it!!.readBytes()) }, equalTo(Success("hello")))
        ...
        assertThat(s3Bucket.get(key).successValue().let { String(it!!.readBytes()) }, equalTo(Success("there")))
